...
- Output: ranked list + CSV/JSON + Plotly HTML charts. Scores are negative for stronger shorts; SOL memecoins bias further negative if enabled.

Setup
- Host: Windows 11 + WSL2 (Ubuntu), Python 3.13, `uv`, VS Code. Workspace root `/mnt/d`.
- Install deps: `uv pip install -r requirements.txt`.
- Run (quick): `uv run python main.py --top 15 --refresh --export-pairs [--pairs JTOUSD,BONKUSD] [--plotly-top 5]`.
- Run (long with timeout): `./run_main.sh --refresh` (activates `.venv`, loads `.env`, updates deps with `uv`, 60-minute timeout; passes through extra flags).
...
- Scoring: `src/scoring/coin_ranker.py` (indicator votes: EMA slopes, returns YTD/30d/7d, N-bars up/down on last 5 bars; missing indicators skipped; weighted by timeframe multipliers; more negative = better short; no SOL bonus at present).
...
- Aggregation: monthly/daily/4h multipliers (4x/2x/1x) from `config/indicator_config.py`; scores are indicator totals per timeframe; no hard fail—rank all; Solana base can add negative bias.
